...
export default function StagedDatum({ tags, addActiveDatum }: { tags: TagProps[], addActiveDatum: (tags: TagProps[]) => void }) {
  ...
  return (
    <footer className='flex flex-col relative items-center justify-between bottom-0 h-auto w-full border-t border-neutral-700 bg-black'>
      ...
      {activeTag && <TagValueMenu isVisible={isMenuVisible === 'value'} nameTag={activeTag} tags={tags} onClick={() => addValueToActiveTag(i)} />}
            ...
            <div className='inline-flex relative'>
              ...
              <form onSubmit={addTagToStaged} className='flex items-center justify-center'>
                <NameInput
                  ...
                  onFocus={() => beginCreateActiveTag(tagInputValues.length - 1)}
                  onBlur={(e) => endCreateActiveTag(e, tagInputValues.length - 1)}
                  onChange={updateNameInput}
                  tagNameTag={activeTag}
                />
                {tagInputValues[tagInputValues.length - 1] && <button className={`flex items-center rounded-tr rounded-br justify-center w-[30px] h-[30px] text-lg text-black ${focusedInputIndex === stagedTags.length ? 'bg-white' : 'bg-neutral-700'}`} onClick={addTagToStaged}><FaPlus /></button>}
              </form>
            </div>
          </div>
        </div>
